...
module Redmine
  module Pagination
    ...
    module Helper
      ...
      def pagination_links_fill(*args)
        pagination_links_fill_each(*args) do |text, parameters, options|
          if block_given?
            yield text, parameters, options
          else
            link_to text, params.merge(parameters), options
      ...
      def pagination_links_fill_each(paginator, count=nil, options={}, &block)
        options.assert_valid_keys :per_page_links
