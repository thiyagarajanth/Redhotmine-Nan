			...
			"Ok": function() {
				...
				if(holidayDate != "" && validateDate(holidayDate))
				{
					...
					if(!checkDuplicate(listBox,holidayDate)){
                        ...
                        if (document.getElementById("time_zone_select_time_zone").value != ""){
                            //alert(document.getElementById("time_zone_select_time_zone").value)
                            timezone = " | " + document.getElementById("time_zone_select_time_zone").value;
                        }
                        else
                        {
                            timezone = " | " + " ";
                        }
						opt.text = holidayDate + desc + timezone;
						opt.value = holidayDate +  desc + timezone;
...
function updateCustFldDD(currCFDD,anotherCFDD)
{
	...
	function showDialog(action)
	{
        ...
        var holidayTimeZone = document.getElementById("time_zone_select_time_zone");
		holiDayDesc.value = "";
		if('Add' == action)
		{	
		...
		{				
			...
			holidayDate.value = listboxArr[0];
			selectedDate=listboxArr[0];
			if(listboxArr[1] != null)
				holiDayDesc.value = listboxArr[1];
				holiDayAction = action;
			$( "#holiday-dlg" ).dialog( "open" )	
		}
		...
		for(i=0; i<listbox.options.length; i++)
		 {
			...
			if(newValue == listboxArr[0] && selectedDate!=newValue)
			{
				isDuplicate=true;
			}
		 }
