...
//$(function() {
    ...
    $( "#notes-dlg" ).dialog({
        ...
        buttons: {
            "Ok": function() {
                if ($.trim(e_notes.val()).length > 0) {
                    $( this ).dialog( "close" );
                    $( "#hidden_wk_reject" ).val('r');
                    $( "#wktime_notes" ).val(e_notes.val());
                    var txt = $.trim(e_notes.val());
                    $("form#wktime_edit").append('<input name= "wktime_notes" id="wk_notes" value="" style="display:none" />');
                    var input_box = $("#wk_notes");
                    input_box.val(input_box.val() + txt);
                    $("#wktime_edit").submit();
                }
                else{
                    $('#notes-dlg label').append("<span style='float:right;color:red'>should not be empty</span>")
                    return false
                }
            },
            Cancel: function() {
    ...
    $( "#notes-dlg_reject" ).dialog({
        autoOpen: false,
        resizable: false,
        modal: true,
        buttons: {
            "Ok": function() {
                if ($.trim(e_note.val()).length > 0) {
                    $( this ).dialog( "close" );
                    $( "#hidden_wk_reject" ).val('n');
                    $( "#wktime_note" ).val(e_note.val());

                    var txt = $.trim(e_note.val());
                    $("form#wktime_edit").append('<input name= "wktime_notes" id="wk_note" value="" style="display:none" />');
                    var input_box = $("#wk_note");
                    input_box.val(input_box.val() + txt);
                    $("#wktime_edit").submit();
                }
                else{
...
function showNotesReject() {
    ...
    posX = $(rejectBtn).offset().left - $(document).scrollLeft() - width + $(rejectBtn).outerWidth();
    posY = $(rejectBtn).offset().top - $(document).scrollTop() + $(rejectBtn).outerHeight();
    $("#notes-dlg_reject").dialog({width:width, height:height ,position:[posX, posY]});
    $( "#notes-dlg_reject" ).dialog( "open" );
    //return false so the form is not posted
    return false;
}
...
function postDeleteRow(result, row, days, deleteMsg){
    ...
    if(result == "OK"){
        ...
        if(rowCount - (headerRows + footerRows - 1) <= 2){
            var  check_boxes = $('.approval-checkbox')
            if(check_boxes)
            {
                check_boxes.remove();
            }
...
//Calculates and fills the total hrvalidateMinhour
function updateTotalHr(day)
{
    ...
    if (tot_min > 0)
    {
        totTime = tot_Hr + ":projectChanged" + tot_min;
    }
    else
    {
        totTime = tot_Hr + ":00";
...
function validateMinhour(minHour,nonWorkingDay){
    ...
    if (minHour!=0 && !isNaN(minprojectChangedHour)){
        for (i=1;i<=7;i++){
